...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["exam_pin"])) {
        ...
        if ($result && mysqli_num_rows($result) <= 0) {
            ...
            //     $exam_pinError = "You did not take this exam";
            //     $isValid = false;
            //     $result_status;
            // } else {
            // Check if exam_pin exists in the exam_registration table for this exam
            $examPinQuery = "SELECT * FROM exam_registrations WHERE exam_id = ? AND exam_pin = ? AND result_status = ?";
            $stmt = mysqli_prepare($conn, $examPinQuery);
            mysqli_stmt_bind_param($stmt, "sss", $examId, $exam_pin, $result_status);
            mysqli_stmt_execute($stmt);
            $result = mysqli_stmt_get_result($stmt);
    ...
    if ($isValid) {
        ...
        $_SESSION['msg'] = "Result Checked Successfully";
        // Redirect to the test page
        header("Location: https://exams.codefest.africa/result?id={$examId}");
        exit();
    }
}
        ...
        <div class="col-lg-5 p-0">
            <div class="card border">
                ...
                <div class="card-header text-center p-4 bg-primary">
                    <h1 class="m-0 text-light">Check Result</h1>
                </div>
                ...
                <div class="card-body rounded-bottom py-5 px-sm-5 px-3 bg-light">
                    <form method="POST" class="needs-validation" novalidate>
                        ...
                        <div>
                            <button class="btn btn-dark btn-block border-0 py-3" type="submit" formtarget="_blank">Check Result</button>
                        </div>
                    </form>
                </div>
            </div>
